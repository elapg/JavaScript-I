...
console.log(lastCar["car_make","car_model"]); Town Car
...
let carYears = [];
for (let i = 0; i < inventory.length; i++) {
     carYears.push(inventory[i].car_years); 
    }
...
let oldCars = [];
for (let i = 2000; i < inventory.length; i++) {
  oldCars.push(inventory[i].old_Cars); 
 }
console.log(oldCars);
